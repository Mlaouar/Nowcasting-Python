...
def dfm(X, Spec, threshold=1e-5, max_iter=5000):
    ...
    while num_iter < max_iter and not converged:  # Loop until converges or max iter.
        ...
        C_new, R_new, A_new, Q_new, Z_0, V_0, loglik = EMstep(y_est, A, C, Q, R, Z_0, V_0, r, p, R_mat, q, nQ, i_idio,
                                                              blocks)
...
def InitCond(x, r, p, blocks, optNaN, Rcon, q, nQ, i_idio):
    ...
    # Set the first observations as NaNs: For quarterly-monthly aggregate. scheme
    ...
    for i in range(n_b):  # Loop for each block
        ...
        d, v = eigh(np.cov(res[:, idx_iM], rowvar=False))
        ...
        initV_i = np.reshape(
            np.matmul(np.linalg.inv(np.eye((r_i * ppC) ** 2) - np.kron(A_i, A_i)), Q_i.flatten('F').reshape((-1, 1))),
            (r_i * ppC, r_i * ppC))
    ...
    C = np.hstack(
        [C, np.vstack([np.zeros((nM, 5 * nQ)), np.kron(np.eye(nQ), np.array([1, 2, 3, 2, 1]).reshape((1, -1)))])])
    ...
    initViQ = np.matmul(np.linalg.inv(np.eye((5 * nQ) ** 2) - np.kron(BQ, BQ)), SQ.reshape((-1, 1))).reshape(
        (5 * nQ, 5 * nQ))
...
def EMstep(y, A, C, Q, R, Z_0, V_0, r, p, R_mat, q, nQ, i_idio, blocks):
    ...
    for i in range(num_blocks):  # Loop through each block
        if i == 0:
            ...
            bl_idxM = np.hstack(
                [np.hstack([bl_idxM, np.tile(bl[:, [i]], (1, r[0, i]))]), np.zeros((n_bl, r[0, i] * (ppC - 1)))])
    ...
    for i in range(n_bl):  # Loop through unique loadings (e.g. [1 0 0 0], [1 1 0 0])
        ...
        C_new[np.ix_(idx_iM, bl_idxM_ind)] = vec_C.copy().reshape((n_i, rs),
                                                                  order="F")  # CHECK: RESHAPE NEEDS TO BE VERIFIED
        ...
        for j in idx_iQ:
            ...
            C_i_constr = C_i - np.matmul(np.matmul(np.matmul(np.linalg.inv(denom), R_con_i.T),
                                                   np.linalg.inv(
                                                       np.matmul(np.matmul(R_con_i, np.linalg.inv(denom)), R_con_i.T))),
...
def SKF(Y, A, C, Q, R, Z_0, V_0):
    ...
    for t in range(nobs):
        ...
        if Y_t.shape[0] == 0:
            ...
            S["loglik"] = S["loglik"] + .5 * (np.log(np.linalg.det(iF)) - np.matmul(np.matmul(innov.T, iF), innov))[
                0, 0]
...
def FIS(A, S):
    ...
    J_2 = np.matmul(np.matmul(np.squeeze(S["VmU"][nobs - 1, :, :]), A.T),
    ...
    for t in range(nobs)[::-1]:  # Loop through time reverse-chronologically (starting at final period nobs)
        ...
        if t > 0:
            # Update weight
            J_2 = np.matmul(np.matmul(np.squeeze(S["VmU"][t - 1, :, :]), A.T),
